...
/// ```
fn f_sample_size(effect_size: f64, alpha: f64, power: f64, num_groups: usize) -> PyResult<f64> {
    ...
    let f_dist = FisherSnedecor::new(df1, df2).map_err(|_| PyErr::new::<pyo3::exceptions::PyValueError, _>(
        "Failed to create Fisher's F distribution",
    ))?;
    ...
    Ok(n.ceil()) // Rounds up to the next whole sample size
}

/// Python wrapper for the sample size calculation function.
#[pyfunction]
pub fn py_f_sample_size(effect_size: f64, alpha: f64, power: f64, num_groups: usize) -> PyResult<f64> {
    f_sample_size(effect_size, alpha, power, num_groups)
}
